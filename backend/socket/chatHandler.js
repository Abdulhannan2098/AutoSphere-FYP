  ...
  io.on('connection', (socket) => {
    ...
    socket.on('message:read', async (data) => {
      try {
        ...
        // Emit to sender
        io.to(`user:${message.sender.toString()}`).emit('message:read-receipt', {
          messageId,
          conversationId,
          readBy: userId,
          readAt: new Date()
    ...
    socket.on('conversation:mark-read', async (conversationId) => {
      try {
        ...
        console.log(`✓✓ All messages marked as read in conversation ${conversationId} by ${socket.user.name}`);
      } catch (error) {
